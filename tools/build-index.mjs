...
из них не взято. Исключение одно и оно названо ниже.
...
verify.mjs       шлюз листов, восемь проверок на компонент
...
\`node verify.mjs\` — восемь проверок на компонент, шесть из контракта и две сверх него.
...
## Одно заимствование не из Figma

\`fonts.css\` — файлы Roboto в \`data:font/woff2\`, взятые из уже вшитого листа прошлой сборки.
Из Figma их взять нельзя: это бинарный ассет, которого там нет, а сети у шлюза нет по условию.
Значения дизайна из HTML не брались ни одного. В исходнике шрифт лежал двенадцатью блоками —
три подмножества × четыре веса, — и в каждой четвёрке был байт в байт один и тот же файл.
Свёрнуто в три \`@font-face\` с \`font-weight: 100 900\`: **550 КБ → 139 КБ на лист**.
